...
describe("TaskService", () => {
  ...
  describe("addTask", () => {
    ...
    it("should throw an error if title is missing, empty, or not a string", () => {
      expect(() => taskService.addTask({})).toThrow("Title is required and must be a non-empty string");
      expect(() => taskService.addTask({ title: "" })).toThrow("Title is required and must be a non-empty string");
      expect(() => taskService.addTask({ title: "   " })).toThrow("Title is required and must be a non-empty string");
      expect(() => taskService.addTask({ title: 123 })).toThrow("Title is required and must be a non-empty string");
    });
  ...
  describe("getTask", () => {
    ...
    it("should throw an error if the task is not found", () => {
      expect(() => taskService.getTask("non-existent-id")).toThrow("Task with ID non-existent-id not found");
    });
  });

  describe("getTasks (with filtering)", () => {
    beforeEach(() => {
      taskService.addTask({ title: "Task A", status: "pending", priority: "low", description: "First task" });
      taskService.addTask({ title: "Task B", status: "in_progress", priority: "medium", description: "Second task" });
      taskService.addTask({ title: "Task C", status: "completed", priority: "high", description: "Third task" });
    });
    ...
    it("should filter by dueBefore and dueAfter", () => {
      ...
      const resultsBetween = taskService.getTasks({ dueAfter: "2026-07-05", dueBefore: "2026-07-15" });
  ...
  describe("updateTask", () => {
    ...
    it("should throw an error when updating a non-existent task", () => {
      expect(() => taskService.updateTask("invalid-id", { title: "New" })).toThrow("Task with ID invalid-id not found");
    });

    it("should validate updated values correctly", () => {
      const task = taskService.addTask({ title: "Task" });
      expect(() => taskService.updateTask(task.id, { title: "" })).toThrow("Title must be a non-empty string");
      expect(() => taskService.updateTask(task.id, { status: "invalid" })).toThrow("Invalid status");
    });
  });
  ...
  describe("deleteTask", () => {
    ...
    it("should throw an error when deleting a non-existent task", () => {
      expect(() => taskService.deleteTask("invalid-id")).toThrow("Task with ID invalid-id not found");
    });
  });
  ...
  describe("getOverdueTasks", () => {
    it("should return tasks that are past their due date and not completed", () => {
      ...
      taskService.addTask({ title: "Overdue In Progress", dueDate: pastDate, status: "in_progress" });
